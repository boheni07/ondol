...
> 범례: ✅ **확정**(설계 문서에 명시) · 🟡 **권장**(미확정, 결정 필요) · ⚪ **차기**(P1/P2)
...
| 백엔드·DB | PostgreSQL + RLS (Supabase **또는** bkend.ai BaaS) · Prisma | ✅ / 🟡 선택 |
| 인증 | 이메일·비밀번호 + 카카오/네이버 OAuth | ✅ |
| 스토리지 | 객체 스토리지 + Presigned URL + CDN | ✅ |
| 알림 | FCM(푸시) · 이메일(SendGrid/Resend) · SMS | ✅ / ⚪ SMS |
| 캐시 | Redis (권한 캐시) | ⚪ P1 |
...
| 상태/서버데이터 | TanStack Query 🟡 | 서버 상태 캐싱·동기화 (미확정, 결정 필요) |
| 폼 | React Hook Form + Zod 🟡 | 다단계 위자드 검증 (미확정) |
...
WBS 0.6은 **"bkend.ai BaaS 연동 (또는 Supabase 직접)"** — 둘 중 하나를 확정해야 한다.
...
| 인증·스토리지 | 통합 제공 | 통합 제공 |
| 적합성 | RLS·트리거·pgTAP 등 **PostgreSQL 깊은 제어**가 필요한 본 프로젝트에 유리 | 빠른 온보딩 |

> **권장:** RLS·DB 트리거·pgTAP 검증 의존도가 높으므로(`03-erd.md` RLS, WBS 1.2/4.4/11) **Supabase 직접 제어**를 우선 검토. 최종 결정은 팀 인프라 역량에 따른다.
...
|------|------|:---:|
| 앱 푸시 | FCM | ✅ WBS 10.2.1 |
| 이메일 | SendGrid 또는 Resend 🟡 | ✅ 채널 / 🟡 벤더 선택 |
...
| # | 항목 | 옵션 | 권장 |
|---|------|------|------|
| 1 | BaaS | Supabase 직접 / bkend.ai | **Supabase**(RLS 제어) |
| 2 | 서버 상태 관리 | TanStack Query / SWR | TanStack Query |
| 3 | 폼·검증 | React Hook Form + Zod / 기타 | RHF + Zod |
| 4 | 이메일 벤더 | SendGrid / Resend | 비용·도달률 비교 후 |

> 위 항목은 구현 착수(Phase 0) 전에 팀이 확정한다. 확정 시 본 문서를 갱신하고 WBS 변경 이력(`CLAUDE.md`)에 기록한다.

---

> 본 문서는 `08-wbs.md`·`02-data-specification.md`·`03-erd.md`·`07-design-system.md`에 명시된 기술 선택을 종합한 것이다. ✅ 외 항목은 미확정이므로 임의 구현 전 결정 절차를 거친다.
