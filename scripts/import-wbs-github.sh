...
- [ ] **0.2** 기술 스택 확정 문서 작성 `S` `P0` — 프론트(Next.js App Router)·모바일(Expo RN)·백엔드(Supabase/bkend.ai)·DB(PostgreSQL+RLS) 스택 확정 및 근거 문서화. 참고: docs/02-data-specification.md, docs/03-erd.md, docs/10-tech-stack.md
...
- [ ] **1.1.9** `record_files` 테이블 `XS` `P0` — record_files — 기록·자기표현 첨부 파일 메타. storage_path·is_sensitive·mime. 참고: docs/02 §2(record_files), docs/05 §5 파일 첨부
...
### 작업 목록
- [ ] **4.1.1** 권한 부여 API (도메인·수준·기간) `M` `P0` — 권한 부여 — 권한자·도메인(멀티)·수준(read/write/edit, admin은 주보호자 전용·위임 불가)·기간 UPSERT. 참고: docs/02 §2(permissions)·§4 Enum(access_level), docs/05 §2 권한 부여
